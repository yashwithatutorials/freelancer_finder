...
const Details = () => {
  ...
  const BASE_URL = "https://freelancer-finder.onrender.com";
  const BASE_UPLOADS = `${BASE_URL}/uploads/`;

  // Function to properly construct file URLs
  const getFileUrl = (filePath) => {
    if (!filePath) return null;
    if (filePath.startsWith('http')) return filePath;
    return `${BASE_UPLOADS}${filePath}`;
  };
  ...
  useEffect(() => {
    const fetchFreelancers = async () => {
      try {
        ...
        // Ensure proper URL construction for all file paths
        if (current) {
          current.profileImage = getFileUrl(current.profileImage);
          current.resume = getFileUrl(current.resume);
        }
